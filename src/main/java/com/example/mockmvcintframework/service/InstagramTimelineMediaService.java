...
public class InstagramTimelineMediaService {
    ...
    public InstagramPostDTO prepareImage(TimelineImageMedia media, InstagramPostDTOBuilder postBuilder) {
        ...
        if (isEmpty(imageVersionsMetas)) {
            ...
            var imageDetails = ImagePostDetailsDTO.builder()
    ...
    public InstagramPostDTO prepareVideo(TimelineVideoMedia media, InstagramPostDTOBuilder postBuilder) {
        ...
        if (isEmpty(videoVersionsMetas)) {
            ...
            var videoDetails = VideoPostDetailsDTO.builder()
                    .viewCount(media.getView_count())
    ...
    public InstagramPostDTO prepareCarousel(TimelineCarouselMedia media, InstagramPostDTOBuilder postBuilder) {
        var carouselItems = media.getCarousel_media().stream()
                .map(item -> {
                    var itemBuilder = CarouselItemDTO.builder()
                            .parentId(item.getCarousel_parent_id())
                            .originalWidth(item.getOriginal_width())
                            .originalHeight(item.getOriginal_height());

                    return (item instanceof ImageCarouselItem) ? prepareImageCarouselItem((ImageCarouselItem) item, itemBuilder)
                            : (item instanceof VideoCarouselItem) ? prepareVideoCarouselItem((VideoCarouselItem) item, itemBuilder)
                            : itemBuilder.build();
                })
                .toList();

        var carouselDetails = CarouselPostDetailsDTO.builder()
    ...
    private CarouselItemDTO prepareImageCarouselItem(ImageCarouselItem item, CarouselItemDTO.CarouselItemDTOBuilder itemBuilder) {
    ...
    private CarouselItemDTO prepareVideoCarouselItem(VideoCarouselItem item, CarouselItemDTO.CarouselItemDTOBuilder itemBuilder) {
